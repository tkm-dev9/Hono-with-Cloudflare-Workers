...
describe('Todos API', () => {
  ...
  test('Todo 一覧を取得する', async () => {
    const res = await app.fetch(new Request('https://localhost'), env);
  ...
  test('Todoを更新する', async () => {
    ...
    const res1 = await app.fetch(new Request('http://localhost/3', {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
  ...
  test('Todoを削除する', async () => {
    const res1 = await app.fetch(new Request('http://localhost/2', {
      method: 'DELETE'
    }), env);
    expect(res1.status).toBe(204);
